...
static constexpr PypilotNameAlias pypilot_compat_aliases[] = {
...
static constexpr size_t pypilot_compat_aliases_size =
    sizeof(pypilot_compat_aliases) / sizeof(pypilot_compat_aliases[0]);

inline FieldId field_id_from_compat_name(const char* name) {
    if (!name) return FieldId::unknown;
    for (size_t i = 0; i < pypilot_compat_aliases_size; ++i) {
        if (strcmp(pypilot_compat_aliases[i].name, name) == 0) return pypilot_compat_aliases[i].id;
    }
